      ...
      ${
        ...
        paths
          .map(({ params, _updatedAt }: { _updatedAt?: string; params: { category: string[] } }) => {
            ...
            return `
                <url>
                  <loc>${`${process.env.NEXT_PUBLIC_SITE_URL}/blogs/page/${category[0]}`}</loc>
...
export async function getServerSideProps({ res, locale }: { res: NextApiResponse; locale: string }) {
  const records = await sanityClient.fetch(
    `
    {
      "paginationCount": *[_type == "blog.index" && __i18n_lang == $locale][0] {
        paginationCount
      },
      "allPosts": count(*[_type == "blog" && __i18n_lang == $locale]),
      "categories": *[_type == "blog.category" && __i18n_lang == $locale] {
        _updatedAt,
        slug,
        "postsCount": count(*[_type == "blog" && references(^._id) && __i18n_lang == $locale])
      }
    }
  `,
    { locale: locale }
